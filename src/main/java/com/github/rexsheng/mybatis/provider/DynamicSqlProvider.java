...
public class DynamicSqlProvider {
	...
	public <T> String selectByBuilder(QueryBuilder<T> builder) {
		return new SQL(){{
			...
			//分页
			if(sourceTable.getTakeSize()!=null) {
	...
	public <T> String selectCount(QueryBuilder<T> builder) {
		return new SQL(){{
			BuilderConfiguration configuration=builder.getBuiderConfig();
			TableQueryBuilder<?> sourceTable=builder.getTable();
			Boolean singleTable=sourceTable.getJoinList().isEmpty();
				...
				return new SQL() {{
					...
					for(int i=0;i<dataList.size();i++) {
						if(i==0) {
							...
							String tableName=new BuilderConfiguration().getTableNameHandler().apply(element.getClass());
							...
							for(Field field:fields) {
								...
								if(columnName!=null) {
									...
									col=StringUtils.camelCaseToUnderLine(field.getName());
								}
								INTO_COLUMNS(col);
